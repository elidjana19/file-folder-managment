...
export class HeaderComponent {
  ...
  constructor(public dialog:MatDialog, private router:Router, private folderService:FolderServiceService,
    private toastr:ToastrService, private cdr: ChangeDetectorRef, public elRef: ElementRef,
    private clickTrackerService: ClickTrackerServiceService
  ){}

   ngOnInit(){

    this.folderService.getSelectedFolder()
      .subscribe(folder => {
...
// create is OK
  ...
  createFolder(folderName:string){
    ...
    this.folderService.createFolder(folderName, parentId).subscribe(res=>{
      ...
      this.folderService.deleteFolder(this.selectedFolder.id).subscribe({
        next: () => {
          if (this.selectedFolder.parentFolderId) {
            this.folderService.getFolderById(this.selectedFolder.parentFolderId).subscribe(parentFolder => {
      ...
      this.folderService.deleteFile(this.selectedFile.id).subscribe({
        next: () => {
          if(this.selectedFile.folderId){
            this.folderService.getFolderById(this.selectedFile.folderId).subscribe(folder=>{
...
downloadFile(file: any): void {
  this.folderService.downloadFile(file.id).subscribe(blob => {
    let fileType: string;
